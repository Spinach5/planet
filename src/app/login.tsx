...
export default function LoginPage() {
  ...
  const handleLogin = useCallback(async () => {
    ...
    if (!password) { showToast({ message: '请输入密码', type: 'error' }); return; }
        ...
        <ScrollView style={styles.scrollView} keyboardShouldPersistTaps="handled">
          ...
          <View style={[styles.formCard, { backgroundColor: theme.surface }]}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                style={[styles.input, { backgroundColor: theme.backgroundElement, color: theme.text, borderColor: studentIdError ? '#ff4d4f' : 'transparent' }]}
                placeholder="请输入学号" placeholderTextColor={theme.textSecondary}
                value={studentId} onChangeText={(t) => { setStudentId(t.replace(/\D/g, '')); setStudentIdError(false); }}
                keyboardType="numeric" maxLength={20}
              />
              {studentIdError ? <Text style={styles.errorText}>学号格式不正确</Text> : null}
            </View>
            <View style={styles.inputGroup}>
              <ThemedText style={styles.label}>密码</ThemedText>
              <View style={styles.pwdRow}>
                <TextInput
                  style={[styles.input, styles.pwdInput, { backgroundColor: theme.backgroundElement, color: theme.text, borderColor: 'transparent' }]}
                  placeholder="请输入密码" placeholderTextColor={theme.textSecondary}
                  value={password} onChangeText={setPassword} secureTextEntry={!showPassword}
                />
                <TouchableOpacity style={styles.eyeBtn} onPress={() => setShowPassword(!showPassword)}>
                  <MaterialIcon name={showPassword ? 'eye-off' : 'eye'} size={20} color="#666" />
                </TouchableOpacity>
              </View>
            </View>
            <TouchableOpacity style={[styles.loginBtn, loading && { opacity: 0.6 }]} onPress={() => { void handleLogin(); }} disabled={loading}>
            ...
            <TouchableOpacity style={styles.agreementRow} onPress={() => setAgreed(!agreed)}>
              ...
              <Text style={styles.agreementText}>
                我已阅读并同意 <Text style={styles.link} onPress={() => setModalType('agreement')}>用户协议</Text> 和 <Text style={styles.link} onPress={() => setModalType('privacy')}>隐私政策</Text>
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  formCard: { marginHorizontal: 12, marginTop: 12, borderRadius: 20, padding: 20 },
  inputGroup: { marginBottom: 16 },
  label: { fontSize: 14, fontWeight: '600', marginBottom: 8 },
  input: { borderRadius: 8, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 12, fontSize: 16 },
  pwdRow: { flexDirection: 'row', alignItems: 'center' },
  pwdInput: { flex: 1 },
  eyeBtn: { position: 'absolute', right: 12 },
  errorText: { color: '#ff4d4f', fontSize: 12, marginTop: 4 },
  loginBtn: { backgroundColor: '#47a5fd', borderRadius: 20, paddingVertical: 16, alignItems: 'center', marginTop: 8 },
  loginBtnText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  agreementRow: { flexDirection: 'row', alignItems: 'center', marginTop: 16, gap: 8 },
