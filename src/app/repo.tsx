...
import { getContributor, getRepos, getLatestCommit } from '@/service/gitee';
      ...
      <View style={cs.info}>
        <View style={cs.nameRow}>
          <View style={cs.nameWrap}>
            ...
            {contributor.email ? <Text style={cs.email}>{contributor.email}</Text> : null}
          </View>
          <Text style={cs.commits}>{contributor.commits} commits</Text>
        </View>
        <View style={cs.barTrack}>
          <View style={[cs.barFill, { width: `${String(barWidth)}%`, backgroundColor: bgColor }]} />
        </View>
      </View>
    </View>
  );
...
const cs = StyleSheet.create({
  ...
  rank: { width: 22, fontSize: 11, fontWeight: '600', color: '#999', textAlign: 'center' },
  avatar: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 12, fontWeight: '600', color: '#fff' },
  info: { flex: 1 },
  nameRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 },
  nameWrap: { flex: 1 },
  name: { fontSize: 11, fontWeight: '500', color: '#333' },
  email: { fontSize: 10, color: '#999', marginTop: 1 },
  commits: { fontSize: 10, color: '#888' },
  barTrack: { width: '100%', height: 8, backgroundColor: '#f0f0f0', borderRadius: 4, overflow: 'hidden' },
...
export default function RepoPage() {
  ...
  const fetchLatestCommit = useCallback(async (force = false) => {
    ...
    try {
      ...
      // silent
    } finally { setLatestCommitLoading(false); }
  }, []);
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    void Promise.all([fetchContributors(true), fetchRepos(true), fetchLatestCommit(true)])
      ...
      >
        ...
        >
          ...
          <View style={[s.card, sectionBg]}>
            ...
            {loading ? (
              <View style={s.stateWrap}><Text style={s.stateText}>加载中...</Text></View>
            ) : contributors.length === 0 ? (
              <View style={s.stateWrap}><Text style={s.stateText}>暂无贡献数据</Text></View>
            ) : (
          ...
          <View style={[s.card, sectionBg, { marginTop: 16 }]}>
            ...
            {reposLoading ? (
              ...
              <View style={s.stateWrap}><Text style={s.stateText}>暂无仓库数据</Text></View>
            ) : (
                ...
                {repos.map((repo) => (
                  <View key={repo.name} style={s.repoItem}>
                    ...
                    <View style={s.repoInfo}>
                      ...
                      <Text
                        style={s.repoItemUrl}
                        onPress={() => { void copyToClipboard(repo.url); }}
                      >
                        {repo.url}
                      </Text>
                    </View>
...
const s = StyleSheet.create({
  ...
  // Card
  card: { borderRadius: 12, padding: 20, marginHorizontal: 8, marginBottom: 16, backgroundColor: '#fff' },
  repoName: { fontSize: 15, fontWeight: '700', color: '#1a1a1a', marginBottom: 12 },
  urlRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap' },
  ...
  lcSection: { marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#f0f0f0' },
  ...
  // Section
  sectionTitle: { fontSize: 13, fontWeight: '600', color: '#1a1a1a', marginBottom: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  ...
  repoIcon: { width: 36, height: 36, backgroundColor: '#f0f5ff', borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  repoIconText: { fontSize: 12, fontWeight: '700', color: '#2563eb' },
  repoInfo: { flex: 1 },
  repoItemName: { fontSize: 11, fontWeight: '500', color: '#333', marginBottom: 4 },
  repoItemUrl: { fontSize: 10, color: '#2563eb', textDecorationLine: 'underline' },
});
