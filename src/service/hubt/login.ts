...
export async function login(stuID: string, password: string): Promise<LoginResult> {
  ...
  // Note: auth() internally clears cookies before attempting login
  ...
  try {
    ...
    // Auth succeeded (cookies set) but xhid/stuInfo fetch failed.
    // Credentials are correct — mark as logged in with basic info.
    userManager.stuId = stuID;
    userManager.password = password;
    userManager.isLoggedIn = true;
    await userManager.saveToCache();
